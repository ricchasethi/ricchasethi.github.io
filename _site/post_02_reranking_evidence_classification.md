...
This is a mild signal. Its main job is to break ties between two chunks from the same section of the same paper. If it is shorter then more focused chunk should win over the longer one that matches the same terms but buries them in noise.
...
This works, but it does not go far enough. In the example above, "predict" is in the query and in Chunk B. If you treat "predict" as a discriminative query token, then Chunk B partially escapes the penalty as it matched a query token, just not the disease-specific one.

Here is the insight that required debugging to discover: **clinical verbs are not discriminative**. "Predict", "detect", "identify", "assess", "evaluate" etc. these words appear in virtually every biomedical paper regardless of disease area. They carry no signal about *which* disease a paper covers. A lung cancer paper predicts things. A diabetes paper detects things. Using their presence in a chunk as evidence of topical relevance is simply noise.
...
With this set defined, the discriminative tokens for a query are simply:

```python
discriminative = query_tokens - self._GENERIC_BIO_TERMS
```

For the query *"What plasma biomarkers predict Alzheimer's disease?"*, the discriminative tokens are `{"alzheimer"}`. "Plasma", "biomarker", "predict", "disease" are all in `_GENERIC_BIO_TERMS` and get excluded.
...
The fix: add "predict", "detect", "identify", "assess", and all their morphological variants to `_GENERIC_BIO_TERMS`. This is the second-stage insight: the generic set must include not just domain nouns but also the clinical verbs that appear in any disease-area query. After this addition, the diabetes paper's discriminative match dropped to zero. `entity_factor = 0.15`. It left the top 5.

**The lesson:** When you observe that an off-topic paper survives an entity penalty at higher-than-expected strength, check whether it matched a query token that *should* have been classified as generic. The diagnostic question is: *would a paper about a completely different disease also contain this token?* If yes, it belongs in `_GENERIC_BIO_TERMS`.
...
Contradictory is checked first. A chunk that matches a contradictory pattern is classified as `contradictory` regardless of whether it also matches a direct pattern. Thats because a chunk can simultaneously describe a positive finding and then qualify or refute it. Labelling it `direct` when it contains an explicit contradiction would bury the caveat.

If no contradictory pattern matches, direct patterns are tested. A match on `p < 0.001` or `hazard ratio` is nearly always a quantitative result and the chunk is `direct`. If neither pattern family matches, the chunk defaults to `indirect`.
...
**The patterns are surface-level.** A chunk that uses "suggest" is labelled `direct` because "suggest" appears in `DIRECT_PATTERNS`. But authors use "suggest" both to report their own results ("our data suggest X") and to speculate about mechanisms ("this may suggest X"). The former is direct evidence; the latter is not. Distinguishing them requires parsing the sentence structure, not just matching a verb.
